...
jint JNI_OnLoad(JavaVM* vm, void* reserved) {
  ...
  if (vm->GetEnv(reinterpret_cast<void**>(&env), JNI_VERSION) != JNI_OK) {
    std::cerr << "vm->GetEnv error" << std::endl;
    return JNI_ERR;
  }
...
(JNIEnv *env, jobject obj) {
  ...
  return (jlong)face_detector_handler;
}
...
(JNIEnv *env, jobject obj, jstring model_path) {
  ...
  return (jlong)shape_predictor_handler;
}
...
(JNIEnv *env, jobject obj, jstring model_path) {
  ...
  return (jlong)face_embedding_handler;
}
...
(JNIEnv *env, jobject obj, jlong shapePredictorHandler, jlong faceDetectorHandler, jbyteArray imgdata, jint h, jint w) {
  ...
  try {
    pose_model = shape_predictor_handler->getShapePredictorModel();
  }
  catch (exception& ex) {
